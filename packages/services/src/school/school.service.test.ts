...
describe('schoolService', () => {
	...
	it('deve buscar uso de personagens do dashboard com defaults', async () => {
		apiMock.get.mockResolvedValueOnce({
			data: { topCharacterName: 'Anita', slices: [], totalPlays: 0 },
		});

		await schoolService.getDashboardCharacterUsage();
	...
	it('deve buscar NPS do dashboard por escola', async () => {
		apiMock.get.mockResolvedValueOnce({
			data: { totalResponses: 1, slices: [], viewMode: 'by_school' },
		});

		await schoolService.getDashboardNps({
			gameSlug: 'memory-game',
			schoolId: 'school-1',
		});
	...
	it('deve buscar NPS do dashboard com defaults', async () => {
		apiMock.get.mockResolvedValueOnce({
			data: { totalResponses: 0, slices: [], viewMode: 'by_school' },
		});
	...
	it('deve usar limit padrao ao buscar top usuarios por escola', async () => {
		apiMock.get.mockResolvedValueOnce({ data: [] });

		await schoolService.getDashboardTopUsers({ schoolId: 'school-1' });

		expect(apiMock.get).toHaveBeenCalledWith('/schools/dashboard/top-users', {
			params: { gameSlug: 'memory-game', limit: 10, schoolId: 'school-1' },
		});
	});
