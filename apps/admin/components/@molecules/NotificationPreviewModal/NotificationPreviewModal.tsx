...
export const NotificationPreviewModal = () => {
  ...
  const targetType = (values.targetType ?? 'GLOBAL') as NotificationTargetType;
    ...
    >
      ...
      <Descriptions column={1} size="small">
        <Descriptions.Item label="Público alvo">
          <Tag color={targetTypeColors[targetType]}>{targetTypeLabels[targetType]}</Tag>
        </Descriptions.Item>
        ...
        {values.userId && (
          <Descriptions.Item label="Usuário ID">{values.userId}</Descriptions.Item>
        )}
      </Descriptions>
    </Modal>
  );
